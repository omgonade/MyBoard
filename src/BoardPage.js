...
const BoardPage = () => {
  ...
  const retrievePosts = () => {
    const savedPosts = localStorage.getItem(`boardPosts_${boardName}`);
    if (savedPosts) {
      setPosts(JSON.parse(savedPosts));
    }
  };
  ...
  const handlePublishPost = () => {
    const newPost = { boardId: boardName, subject, image, content };
    setPosts([...posts, newPost]);
    closeCreatePostModal();
    localStorage.setItem(`boardPosts_${boardName}`, JSON.stringify([...posts, newPost]));
  };
  ...
  const handleDeletePost = (index) => {
    ...
    localStorage.setItem(`boardPosts_${boardName}`, JSON.stringify(posts));
  };

  const filteredPosts = posts.filter((post) => post.boardId === boardName);
        ...
        {filteredPosts.map((post, index) => (
          <div className='post' key={index}>
            ...
            <img
              ...
              style={{ height: '50px', width: '50px', marginLeft: '90%', cursor: 'pointer' }}
              alt='Delete'
            />
          </div>
        ))}
